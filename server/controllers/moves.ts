...
async function getApparatusMoves(req: Request, res: Response): Promise<void> {
  try {
    const { apparatus } = req.params;
    const moves: IMove[] = await Move.find({ apparatus })
    res.status(200);
    res.send(moves);
  } catch (error) {
    console.error(error);
...
export default { getAllMoves, getApparatusMoves }